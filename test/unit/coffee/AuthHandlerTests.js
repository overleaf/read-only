...
describe('AuthHandler', function() {
	...
	describe("login", function() {
		beforeEach(function() {
			...
			this.db.users.findOne.withArgs({ email: this.email }).yields(null, this.user);
			this.bcrypt.compare.yields(null, false);
			return this.bcrypt.compare.withArgs(this.password, this.user.hashedPassword).yields(null, true);
		});

		it("checks the password and returns the user id", function(done) {
			return this.AuthHandler.login(this.email, this.password, (err, userId) => {
				if (err != null) {
					return done(err);
				}
		...
		it("is case-insensitive relative to the email", function(done) {
			return this.AuthHandler.login("User@Example.Com", this.password, (err, userId) => {
				if (err != null) {
					return done(err);
				}
				expect(userId).to.equal(this.userId);
		...
		it("throws an AuthenticationError if the user doesn't exist", function(done) {
			return this.AuthHandler.login("incorrect@email.com", this.password, err => {
	...
	describe("oneTimeLogin", function() {
		beforeEach(function() {
			...
			this.db.users.findOne.yields(null, null);
			return this.db.users.findOne.withArgs({ email: this.email }).yields(null, this.user);
		});

		it("picks an unused token, uses it and returns the user id", function(done) {
			return this.AuthHandler.oneTimeLogin(this.email, this.token, (err, userId) => {
				if (err != null) {
					return done(err);
				}
				expect(this.db.oneTimeLoginTokens.findOne).to.have.been.calledWith(sinon.match({
					usedAt: { $exists: false }
				}));
		...
		it("is case-insensitive relative to the email", function(done) {
			return this.AuthHandler.oneTimeLogin("User@Example.Com", this.token, (err, userId) => {
				if (err != null) {
					return done(err);
				}
				expect(userId).to.equal(this.userId.toString());
		...
		it("throws an AuthenticationError when the token is the wrong length", function(done) {
			return this.AuthHandler.oneTimeLogin(this.email, "bad-token-too-long", err => {
				expect(err).to.be.instanceof(Errors.AuthenticationError);
				return done();
			});
		});
		...
		it("throws an AuthenticationError when the email has no token", function(done) {
			return this.AuthHandler.oneTimeLogin('unknown@email.com', this.token, err => {
				expect(err).to.be.instanceof(Errors.AuthenticationError);
				return done();
			});
		});
	...
	return describe("generateOneTimeLoginToken", function() {
		...
		return it("throws a UserNotFoundError when the user does not exist", function(done) {
			...
			return this.AuthHandler.generateOneTimeLoginToken('not-a-user@example.com', (err, token) => {
